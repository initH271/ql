...
OPENAPI_CHAT_COMPLETIONS = 'https://gpt.wf/v1/chat/completions'
...
def chatgpt_answer_question(question_name, answer_str, chatgpt_index=0, knowledge_points='', tips=''):
    ...
    def requests_post_0(question):
        try:
            ...
            # 连接超时时间为3s，读取超时时间为3s
            response = requests.post(url=OPENAPI_CHAT_COMPLETIONS, headers=headers, json=_json,
                                     stream=True, timeout=(10, 10))
            result = ''
            for chunk in response.iter_content(chunk_size=1024):
                chunk = [
                    json.loads("{" + i.replace('data:', '"data":') +
                ...
                for data in chunk:
                    ...
                    data = data.get('data')
                    if data is None or data.get('choices')[0].get('finish_reason') == 'stop':
                        return result
...
class CCBLife:
    def __init__(self, ck, index):
        ...
        self.Component_headers = {'x-csrf-token': '',
    ...
    def receive_daily_earnings(self):
        ...
        level_reward_type = response_dict.get(
            'data').get('zhcRewardInfo').get('rewardType')
        url = "https://m3.dmsp.ccb.com/api/businessCenter/mainVenue/receiveLevelReward"
        _json = {"level": level, "rewardId": reward_id,
                 "levelRewardType": level_reward_type}
        response = self.session.post(
            url, headers=self.businessCenter_headers, json=_json)
        response_dict = response.json()
        dump_msg(response_dict.get('message'))
    ...
    def get_component_headers(self):
        ...
        if csrf_token_match and authorization_match:
            csrf_token, authorization = csrf_token_match.group(
                1), authorization_match.group(1)
    ...
    def elderly_walking(self):
        ...
        task_list = response_dict.get('data', {}).get(
            'acttask', {}).get('limit_time')
        ...
        if surplus != '0':
            response = self.session.post(
                go_url, headers=self.Component_headers)
            response_dict = response.json()
        ...
        response = self.session.get(
            tasklist_url, headers=self.Component_headers)
        response_dict = response.json()
        rewards = response_dict.get('data', {}).get(
            'map', {}).get('config').get('node')
    ...
    def cross_borders(self):
        ...
        if response_dict.get('data').get('is_answer') == 0:  # 今天未答题
            ...
            response = self.session.get(url, headers=self.Component_headers)
            response_dict = response.json()
            level = response_dict.get('data').get(
                'list')[0].get('level')  # 获取关卡id
    ...
    def get_expiration_information(self):
        ...
        response = self.session.post(
            url, headers=self.businessCenter_headers, json=_json)
        response_dict = response.json()
        expire_date = response_dict.get('data').get(
            'userCCBeanExpiredInfo').get('expireDate')
    ...
    def consumer_protection_knowledge_test(self):
        ...
        for _ in range(answer_num):
            ...
            response = self.session.post(
                url, headers=self.Component_headers, json=_json)
            response_dict = response.json()
            question_id = response_dict.get(
                'data').get('next').get('questionId')
    ...
    def come_with_me_to_break_through_answer_question(self, bout_id, level_id, index):
        ...
        params = {"boutId": bout_id, "levelId": level_id,
                  "questionNo": str(index + 1)}
        response = self.session.get(
            url, params=params, headers=self.businessCenter_headers)
        response_dict = response.json()
        if response_dict.get('status') != 'success':
            return False
        # dump_msg(response_dict)
    ...
    def statistics_exp_today(self):
        ...
        for i in range(10):
            if not _signal:
                break
            _json = {"queryTime": formatted_date,
                     "pageNumber": i + 1, "pageSize": 20}
